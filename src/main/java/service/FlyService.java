...
public class FlyService {
    ...
    public Flight planeCalculate (AirplaneCharacteristics characteristics, List<WayPoint> wayPoints, Airplane airplane) {
    ...
    public List<TemporaryPoint> flyCheck(AirplaneCharacteristics characteristics, List<WayPoint> wayPoints, Airplane airplane, Flight flight) {
        maxSpeedCheck(characteristics, wayPoints);
        ArrayList<TemporaryPoint> temporaryPoints = new ArrayList<>();
        if (wayPoints.get(0).getHeight() != airplane.getPosition().getHeight() || wayPoints.get(0).getSpeed() != airplane.getPosition().getSpeed()
        || wayPoints.get(0).getLatitude() != airplane.getPosition().getLatitude() || wayPoints.get(0).getLongitude() != airplane.getPosition().getLongitude()) {
            ...
            System.out.println("height " + airplane.getPosition().getHeight() + ", speed " + airplane.getPosition().getSpeed()
            + ", latitude " + airplane.getPosition().getLatitude() + ", longitude " + airplane.getPosition().getLongitude() + ", course " + airplane.getPosition().getCourse());
        } else {
    ...
    private void maxSpeedCheck(AirplaneCharacteristics characteristics, List<WayPoint> wayPoints) {
        for (WayPoint point : wayPoints) {
            if (point.getSpeed() > characteristics.getMaxSpeed()) {
                throw new RuntimeException("Selected speed " + point.getSpeed() + " bigger that possible " + characteristics.getMaxSpeed());
            }
        }
    }
